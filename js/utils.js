function rectangularCollision({rectangular1, rectangular2}) {
    return (
             rectangular1.attackBox.position.x + rectangular1.attackBox.width + rectangular1.attackBox.offset.x >= 
             rectangular2.position.x - rectangular2.width && // Width of the attackBox
             rectangular1.attackBox.position.x + ( rectangular1.width / 2 ) <= 
             rectangular2.position.x && // 要取rectangular1的position X (最左邊) 跟rectangular2的position Y + width (會等於他的最右邊) 並比較rectangular1的左邊有沒有超過rectangular2的右邊 <= 所以沒有就是true  
             ...
             rectangular1.attackBox.position.x >= // right edge of player's position is greater than the right edge of enemy's position
             rectangular2.position.x + ( rectangular2.width / 2 ) && 
             rectangular1.attackBox.position.y + rectangular1.attackBox.height >= 
